...
function generateCodeBase64(code) {
    const codeBase64 = code.toString('base64')
    return `/** WASM code encoded with BASE64. */\nexport const CODE_BASE64 = "${codeBase64}"\n`
}
...
async function patchIndexJs() {
    let text = await fs.promises.readFile('pkg/lanczos_resampler.js', 'utf-8')
    text = text.replaceAll('./lanczos_resampler_bg.wasm', './code.wasm')
    await fs.promises.writeFile('pkg/lanczos_resampler.js', text)
}

try {
    await fs.promises.rename('pkg/lanczos_resampler_bg.wasm', 'pkg/code.wasm')
} catch (e) {
    if (e.code !== 'ENOENT') {
        throw e
    }
